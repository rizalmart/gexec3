...
#define DEFAULT_CMD_SU "sudo -A \"%s\""
...
void check_config_folder() {
	...
	//printf("Check for settings folder...");
	
    char *xdg_data_home = getenv("XDG_CONFIG_HOME");
    char gexec_path[512];

    // Fallback to $HOME/.local/share if XDG_CONFIG_HOME is not set
    if (xdg_data_home == NULL) {
        ...
        snprintf(gexec_path, sizeof(gexec_path), "%s%s/gexec", home, DEFAULT_DATA_HOME);
    } else {
        snprintf(gexec_path, sizeof(gexec_path), "%s/gexec", xdg_data_home);
    }

    // Check if the directory exists
    struct stat st;
    
    if (stat(gexec_path, &st) == 0 && S_ISDIR(st.st_mode)) {
        //printf("Directory '%s' already exists.\n", gexec_path);
        return;
    }

    // Directory does not exist, attempt to create it
    if (mkdir(gexec_path, 0755) == 0) {
...
void settings_create() {
	...
	filename = (char *)malloc(sizeof(char) * (strlen(getenv("XDG_CONFIG_HOME")) + 15 + 1) );

	sprintf(filename, "%s/gexec/gexec-settings.cfg", getenv("XDG_CONFIG_HOME"));
...
GList *history_read() {
	...
	filename = (char *)malloc(sizeof(char) * (strlen(getenv("XDG_CONFIG_HOME")) + 15 + 1) );
	sprintf(filename, "%s/gexec/gexec_history", getenv("XDG_CONFIG_HOME"));
...
gboolean ui_combo_command_cb_key_press_event(GtkWidget *widget, 
	...
	if (event->keyval == GDK_KEY_Tab) {
		...
		if (task->tabcomp_matches != NULL) {
			...
			//gtk_editable_set_position(GTK_ENTRY(widget), -1);
...
int main(int argc, char *argv[]) {
	...
	gtk_window_set_title(GTK_WINDOW(win_main), "gExec: Run Program");
	gtk_window_set_default_size(GTK_WINDOW(win_main), 400, 77);
	g_signal_connect(win_main,
			"delete_event",
			G_CALLBACK(ui_win_main_cb_delete_event),
			NULL);

	// Command 
	hbox_command = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 5);
	lbl_command = gtk_label_new("Run command:");
	combo_command = gtk_combo_box_text_new_with_entry();
	//gtk_combo_box_set_active (GTK_ENTRY(gtk_bin_get_child(GTK_BIN(combo_command))));
	//gtk_combo_box_set_active(GTK_COMBO_BOX(combo_command),FALSE);
	gtk_combo_box_set_active(GTK_COMBO_BOX(combo_command), 1);

	g_signal_connect(GTK_ENTRY(gtk_bin_get_child(GTK_BIN(combo_command))),
			"activate",
	...
	if (task->history != NULL) {
		//gtk_combo_set_popdown_strings(GTK_COMBO_BOX(combo_command), task->history);
		
		// Create a GtkListStore
		GtkListStore *store = gtk_list_store_new(1, G_TYPE_STRING);

		// Populate the store from the GList
		GList *iter;
		for (iter = task->history; iter; iter = iter->next) {
		gtk_list_store_insert_with_values(store, NULL, 0, 0, iter->data, -1);
		}

		// Set the model
		gtk_combo_box_set_model(GTK_COMBO_BOX(combo_command), GTK_TREE_MODEL(store));
	...
	// Buttons 
	hbox_buttons = gtk_box_new(FALSE, 0);

	btn_ok = gtk_button_new_with_label("OK");
	btn_cancel = gtk_button_new_with_label("CANCEL");
	
	GtkWidget *ok_icon = gtk_image_new_from_icon_name("dialog-ok", GTK_ICON_SIZE_BUTTON);
	GtkWidget *cancel_icon = gtk_image_new_from_icon_name("dialog-cancel", GTK_ICON_SIZE_BUTTON);

	gtk_button_set_image(GTK_BUTTON(btn_ok), ok_icon);
	gtk_button_set_image(GTK_BUTTON(btn_cancel), cancel_icon);

	g_signal_connect(btn_ok,
			"clicked",
	...
	//gtk_window_set_deletable(win_main, FALSE)
